...
ros::Publisher imu("accx", &imu_msg);
ros::Publisher imu("accy", &imu_msg);
ros::Publisher imu("accz", &imu_msg);
ros::Publisher imu("aglx", &imu_msg);
ros::Publisher imu("agly", &imu_msg);
ros::Publisher imu("aglz", &imu_msg);
ros::NodeHandle nh;
...
void loop() {
  ...
  if(millis() - timer > 1000){
    String accX = mpu6050.getAccX();
    String accY = mpu6050.getAccY();
    String accZ = mpu6050.getAccZ();
    String aglX = mpu6050.getAngleX();
    String aglY = mpu6050.getAngleY();
    String aglZ = mpu6050.getAngleZ();
